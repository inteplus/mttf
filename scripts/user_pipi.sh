...
if [ $(id -u) -ne 0 ]; then
  echo "WARNING: As of 2026-01-24, you need to activate a virtual environment (e.g. uv venv) before pip-installing wml packages."
  wml_nexus.py uv pip install --allow-insecure-host localhost --index $pipi_url --index-strategy unsafe-best-match $@
else
